...
void initialize(void)
{
	...
	const GLchar *vertexShaderSourceCode =
		...
		"pos.x += dn * noise(q + vec3(12., 3., 16.)) - dn / 2.;"\
		...
		"vert.z -= -0.2f * t * t;"\
		"transparacy = 1.0 - t / ParticleLifeTime;"\
		"gl_PointSize = mix(0.1f, 5f, t/ParticleLifeTime)+4.0f;"\
		"}"\
		"else"\
		"{"\
		"vert = vPosition;"
		...
		"fMapped = map_flame_s(vec3(vPosition));"\
		"gl_Position = pos;"\
		"pos_out.x = gl_Position.x;"\
		"pos_out.y = gl_Position.y;"\
		"pos_out.z = gl_Position.z;"\
		"out_time = t;"\
		"}";
	...
	const GLchar *fragmentShaderSourceCode =
		...
		"uniform sampler3D u_sampler_noise;"\
		"uniform float ParticleLifeTime;"\
		"float flameVar;"
		"in float fMapped;"\
		"in vec3 pos_out;"\
		"vec3 getDensColor(vec3 pos)"\
		"{"\
		"float f = fMapped;"\
		"float d = 1.;"\
		"vec3 scol = vec3(0.0f,0.0f,0.0f);"\
		"flameVar = sin(out_time*0.55) + 0.56*sin(out_time*0.134) + 0.22*sin(out_time*0.095);"\
		"d = clamp(f, 0., 10.);"\
		...
		"scol += d * flameColor;"\
		"return clamp(scol*0.07, 0., 1.5);"\
		"}"\
		"void main(void)"\
		"{"\
		"if (out_time > ParticleLifeTime)"\
		"{"\
		"discard;"\
		"}"\
		"vec4 noisevec = texture(u_sampler_noise, (cam_position + u_Offset));"\
		"float intensity = (noisevec[0] + noisevec[1] + noisevec[2] + noisevec[3] + 0.03125) * 1.5;"\
		"vec3 color = mix(u_SkyColor, u_CloudColor, intensity);"\
		"float alpha = 0.5f;"\
		"if (out_time > alpha * ParticleLifeTime)"\
		"{"\
		"color = mix(vec3(alpha, alpha, 0.0), u_CloudColor, intensity);"\
		"alpha += 0.3f;"\
		"}"\
		"if (out_time > alpha * ParticleLifeTime)"\
		"{"\
		...
		"alpha += 0.1f;"\
		"}"\
		"vec3 flameCol = getDensColor(pos_out);"
		"FragColor = vec4(color, 0.0);"\
		"FragColor.a = transparacy;"\
		"}";
